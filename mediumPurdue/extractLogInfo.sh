...
############### CENTER OF MASS ###############
cat $PFAD | grep 'Centre of mass' | cut -d' ' -f3- | tr -d '(' | tr -d ')' | sed -e 's/[\\ \\]/       /g' > centreOfMassTemp.dat


############### CENTER OF MASS ###############
cat $PFAD | grep 'Total' | cut -d' ' -f3- | tr -d '(' | tr -d ')' | sed -e 's/[\\ \\]/       /g' > forcesTemp.dat
...
paste Time.dat centreOfMassTemp.dat > data/centreOfMassTemp.dat
sed -i '1i#Time, x, y, z' data/centreOfMassTemp.dat
head -n -$EXCL data/centreOfMassTemp.dat > data/centreOfMass.dat

paste Time.dat forcesTemp.dat > data/forcesTemp.dat
sed -i '1i#Time, x, y, z' data/forcesTemp.dat
head -n -$EXCL data/forcesTemp.dat > data/forces.dat
...
rm centreOfMassTemp.dat
rm data/centreOfMassTemp.dat
rm forcesTemp.dat
rm data/forcesTemp.dat
rm plot/*.png
